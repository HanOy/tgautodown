...
export async function handlePhoto(ctx, msg, info) {
  const dir = ctx.saveDir('photos');
  const savePath = path.join(dir, info.fileName);
  await ctx.reply(msg, `正在下载照片: ${info.fileName}\n- 文件大小: ${fmtSize(info.fileSize)}\n- 消息ID: ${msg.id}`);
